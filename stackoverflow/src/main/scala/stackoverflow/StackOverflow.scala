...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    ...
    // filter answers
    val answers: RDD[(Int, Posting)] = postings.filter(p => p.postingType == 2).map(p => (p.parentId.get, p))
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.values
      ...
      .map(tuple => (tuple._1, answerHighScore(tuple._2.toArray[Posting])))
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    cachedScored.map(p => (firstLangInTag(p._1.tags, langs), p._2))
      .filter(tuple => tuple._1.nonEmpty)
      .map(tuple => (tuple._1.get * langSpread, tuple._2))
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double = 100.0d * grouped(langIndex) / vs.size // percent of the questions in the most common language
